 ...
 * them: `wait` answers *immediately* with `{done, finished, jobs}` — the same
 * structured "still running / finished" shape ADR 0092 §5 gives the MCP long-poll
 * — so a caller that wants to keep waiting re-issues it; and nothing here ever
 * cancels a job on a wait expiring, because the ledger never does either.
 */
...
export class ScriptedJobLedger {
  ...
  constructor(jobs: readonly ScriptedJob[] = []) {
  ...
  /**
   * One poll: which of the named jobs have reached a terminal lifecycle, and
   * whether that satisfies the caller's condition. Answered from the state as it
   * stands, never blocked on — a test drives the ledger between polls.
   */
  private answerWait(params: JobWaitParams): ScriptedOperation {
