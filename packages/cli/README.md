...
Stored credentials live in a `0600` file under the OS app-data directory,
keyed by service origin so a production bearer and a dev bearer never collide.

## Targeting a service

Production is compiled in and silent. `--service <url>` or `ACEWORKFLOW_SERVICE`
overrides it with any URL, and an override is always announced so a command can
never quietly hit a non-production backend.
