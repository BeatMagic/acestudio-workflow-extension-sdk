...
} from "@timedomain/create-acestudio-workflow-extension";
...
test("depends on the SDK line this scaffolder was released alongside", async () => {
  ...
  expect(pkg.dependencies["@timedomain/acestudio-workflow-extension-sdk"]).toBe("^1.2.3");
});
...
test("defaults the SDK range to the pinned one", async () => {
  ...
  const pkg = JSON.parse(read(directory, "package.json")) as { dependencies: Record<string, string> };
  expect(pkg.dependencies["@timedomain/acestudio-workflow-extension-sdk"]).toBe(SDK_VERSION_RANGE);
});
...
test("the pinned SDK range tracks the SDK package beside it", async () => {
  const sdk = JSON.parse(
    await readFile(new URL("../../acestudio-workflow-extension-sdk/package.json", import.meta.url), "utf8"),
  ) as { version: string };
